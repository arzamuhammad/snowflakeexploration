...
class BertNLPWrapper(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, datas):
        ...
        for classes in collected_references_df['class-label']:
            ...
            outputs, lhs_ref = self.model(**tokens)
        ...
        # Calculated last hidden state
        keys = [references_df[references_df['topic label'] == i]['text references'][i] for i in range(references_df.shape[0])]
        for i, last_hidden_state in enumerate(last_hidden_states):
            embeddings = last_hidden_state

            mask = attention_mask[i].unsqueeze(-1).expand(embeddings.size()).float()
        ...
        for i, data in enumerate(y_predictions):
            ...
            _y_probs_sub_class = {keys[i]: y_predicted_cosine_similarity[i] for i in range(len(keys))}
            _y_predicted_cosine_similarity = np.argmax(y_predicted_cosine_similarity)

            y_sub_class.append(max(_y_predicted_cosine_similarity))
